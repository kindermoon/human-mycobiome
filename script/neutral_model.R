...
sncm.fit <- function(spp, pool=NULL, stats=TRUE, taxon=NULL){
	...
	require(Hmisc)
	require(stats4)
	require(bbmle)     # mle2 function	
	options(warn=-1)

	#Calculate the number of individuals (organisms) per community
	...
	#m.mle <- mle(sncm.LL, start=list(m=0.1, sigma=0.1), nobs=length(p)) --error in optim
	...
	## Produce 1-alpha confidence intervals for binomial probabilities
...
#============================================================================
	##Results
	if(stats==TRUE){
		fitstats <- data.frame(m=numeric(), m.ci=numeric(), m.se=numeric(), m.pval=numeric(), m.mle=numeric(), maxLL=numeric(), binoLL=numeric(), poisLL=numeric(), Rsqr=numeric(), Rsqr.bino=numeric(), Rsqr.pois=numeric(), RMSE=numeric(), RMSE.bino=numeric(), RMSE.pois=numeric(), AIC=numeric(), BIC=numeric(), AIC.bino=numeric(), BIC.bino=numeric(), AIC.pois=numeric(), BIC.pois=numeric(), N=numeric(), Samples=numeric(), Richness=numeric(), Detect=numeric())
		fitstats[1,] <- c(coef(m.fit), coef(m.fit)-m.ci[1], summary(m.fit)$parameters[2], summary(m.fit)$parameters[4], m.mle@coef['m'], m.mle@details$value, bino.mle@details$value, pois.mle@details$value, Rsqr, Rsqr.bino, Rsqr.pois, RMSE, RMSE.bino, RMSE.pois, aic.fit, bic.fit, aic.bino, bic.bino, aic.pois, bic.pois, N, nrow(spp), length(p), d)
		return(fitstats)
	} else {
		...
		if(is.null(taxon)){
			...
			B <- merge(A, taxon, by=0, all=TRUE)  # all.x + all.y (extra rows will be added to the output)
			row.names(B) <- B[,1]
			B <- B[,-1]
			B <- B[order(B[,1]),]
		}
